...
class Micu_census_model extends CI_Model {
      ...
      function get_allclean(){
      	$this->db->select('micu_id, date_in, date_out, sic, plist, meds, refs, erefs, notes, pcpdx');
...
function get_forms_label($aid, $forms){
      ...
      $this->db->select('micu_id, date_in, date_out, bed, r_id, sr_id, p_id, service, plist, meds');
      switch ($forms){
            case 0:
            ...
            case 4:
                ...
                break;
            case 5:
                $this->db->select('cbc, bloodchem, protime, urine, abg, fecalysis, uchem, culture, imaging, ecg, others, sic');
                break;
            case 6:
                $this->db->select('cbc2, bloodchem2, protime2, urine2, abg2, fecalysis2, uchem2, culture2, imaging2, ecg2, others2, sic');
                break;
            case 7:
...
function edit_dispo_date($aid, $ddate, $dispo, $plist){
    ...
    $file = "micu_admission_log.txt"; 
    $handle = fopen($file, 'a+');
    date_default_timezone_set('Asia/Hong_Kong');
    $data = date('m/d/Y h:i:s a', time()).", IP Add: ".$this->session->userdata('ip_address').", user:".$_SERVER['PHP_AUTH_USER'].", task: edit dispo date id#".$aid.", date_in:".$this->date_in."\r\n"; 
    fwrite($handle, $data);  	
}  
...
function get_micu_census($disp){
       ...
       //MICU admissions in genmed census
       function get_micugm_census($svc){
	      	 $this->db->select('bed, p_id, r_id, sic, service, date_in, date_out, dispo');	  	
       ...
       //MICU service report
       function micu_report($service, $date1, $date2){
				$dispo_count = "COUNT(IF(dispo = 'Admitted', 1, NULL)) AS admitted, 
								...
								COUNT(IF(dispo = 'Admitted to Wards', 1, NULL)) AS award, 
				...
				$age_count = "COUNT(IF((ROUND(DATEDIFF(date_in, patients.p_bday)/365) < 18 ), 1, NULL)) AS f1,
							  ...
							  ROUND(SUM(DATEDIFF(date_in, patients.p_bday)/365)/COUNT(micu_id)) as averageage,
				...
				$minmax_count = "MAX(ROUND(DATEDIFF(date_in, patients.p_bday)/365)) as maxage,
								 ...
								 ";	
				$sql = "SELECT COUNT(micu_id) as total, ".$dispo_count.", ".$hd_count.", ".$m_count.", ".$ed_count.", ".$age_count.", ".$minmax_count." FROM micu_census, patients WHERE micu_census.p_id = patients.p_id AND date_in >= ? AND date_in <= ?";		
	    ...
	    $this->db->join('patients', 'patients.p_id = micu_census.p_id', 'inner');
	    $this->db->join('residents', 'residents.r_id = micu_census.r_id', 'inner');
            $this->db->where($datewhere);
      ...
      function insert_one_admission(){  
	        ...
	        $query = $this->db->insert('micu_census', $data);
            $id = $this->db->insert_id();
            //log insert task in text file
            $file = "micu_admission_log.txt"; 
            $handle = fopen($file, 'a+');
            date_default_timezone_set('Asia/Hong_Kong');
            $data = date('m/d/Y h:i:s a', time()).", IP Add: ".$this->session->userdata('ip_address').", user:".$_SERVER['PHP_AUTH_USER'].", task: insert micu admission id#".$id." MICU bed#:".$data['bed']." patient id#".$data['p_id']."\r\n"; 
            fwrite($handle, $data);  	
		    return $id;		
     }
      //get micu admission details for display
	  function get_admission_data($aid){
		 $this->db->select('r_id, sr_id, p_id, sic, date_in, date_out, source, service, bed, dispo, plist, meds, notes, refs, erefs, pcpdx');
		 $this->db->where('micu_id', $aid);
		 $query = $this->db->get('micu_census');
		 return $query->result();
	 }   
	  ...
	  function edit_one_admission($id){
		  ...
		  $this->db->where('micu_id', $id);
		  $query = $this->db->update('micu_census',$data);
		  ...
		  $this->Patient_model->update_one_plist($data['p_id'], $data['plist']); 
                  //log update task in text file
                     ...
                     $handle = fopen($file, 'a+');
                     date_default_timezone_set('Asia/Hong_Kong');
                     $data = date('m/d/Y h:i:s a', time()).", IP Add: ".$this->session->userdata('ip_address').", user:".$_SERVER['PHP_AUTH_USER'].", task: edit micu admission id#".$id.", patient#:".$data['p_id'].", Micu Bed#:".$data['bed']." dispo:".$data['dispo']."\r\n"; 
                     fwrite($handle, $data);  							  
	  }
          //backup before delete
	  function insert_one_admission_bu($aid){
          $micu_data = $this->Micu_census_model->get_micudata_by_id($aid);
          foreach ($micu_data as $row){
	      ...
	      $micudata = array(
						...
						'others'=>$row->others,
						'cbc2'=>$row->cbc,
						'bloodchem2'=>$row->bloodchem,
						'protime2'=>$row->protime, 
						'urine2'=>$row->urine,
						'abg2'=>$row->abg,
						'fecalysis2'=>$row->fecalysis,
						'uchem2'=>$row->uchem,
						'culture2'=>$row->culture,
						'imaging2'=>$row->imaging,
						'ecg2'=>$row->ecg,
						'others2'=>$row->others,
						'c_notes'=>$row->c_notes,
		  ...
		  $query = $this->db->insert('micu_census_bu', $micudata);	
                  //log backup delete task in text file
                     $file = "micu_admission_log.txt"; 
                     $handle = fopen($file, 'a+');
                     date_default_timezone_set('Asia/Hong_Kong');
                     $data = date('m/d/Y h:i:s a', time()).", IP Add: ".$this->session->userdata('ip_address').", user:".$_SERVER['PHP_AUTH_USER'].", task: backup micu admission id#".$aid."\r\n"; 
                     fwrite($handle, $data);  
		  return $this->db->insert_id();					  
	  }  
//get micu admission data by id
function get_micudata_by_id($aid){
	$this->db->from('micu_census');
	$this->db->where('micu_id', $aid);
	$query = $this->db->get();
	return $query->result(); 
	  ...
	  function delete_one_admission($a_id){
               $this->db->where('micu_id', $a_id);
               $query = $this->db->delete('micu_census'); 
               //log delete task in text file
                     $file = "micu_admission_log.txt"; 
                     $handle = fopen($file, 'a+');
                     date_default_timezone_set('Asia/Hong_Kong');
                     $data = date('m/d/Y h:i:s a', time()).", IP Add: ".$this->session->userdata('ip_address').", user:".$_SERVER['PHP_AUTH_USER'].", task: delete micu admission id#".$a_id."\r\n"; 
                     fwrite($handle, $data);  
               return true; 
	  }	  
   ...
   function edit_home($aid){
	...
	$long8= clean_form_input($this->input->post('long8', TRUE));    
	  ...
	  function count_all_pcpdx($date1, $date2, $dx, $ref){
			 $sql = "SELECT COUNT(*) AS numrows, COUNT(IF(dispo = 'Mortality', 1, NULL)) AS mortality, COUNT(IF(refs LIKE ?, 1, NULL)) AS referred, SUM(DATEDIFF(date_out, date_in)) as totaldays FROM micu_census WHERE (date_in >= ? AND date_in <= ?) AND pcpdx LIKE ?"; 
