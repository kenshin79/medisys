...
function insert_one_admission_bu($aid){
      ...
      //ER service report
       function er_report($service, $date1, $date2){
				...
				$ed_count = "COUNT(IF((DATEDIFF(date_out, date_in) <= 1 AND dispo = 'Admitted to Wards'), 1, NULL)) AS ed1,
							 COUNT(IF((DATEDIFF(date_out, date_in) = 2 AND dispo = 'Admitted to Wards'), 1, NULL)) AS ed2,
							 COUNT(IF((DATEDIFF(date_out, date_in) = 3 AND dispo = 'Admitted to Wards'), 1, NULL)) AS ed3,
							 COUNT(IF((DATEDIFF(date_out, date_in) = 4 AND dispo = 'Admitted to Wards'), 1, NULL)) AS ed4,
							 COUNT(IF((DATEDIFF(date_out, date_in) = 5 AND dispo = 'Admitted to Wards'), 1, NULL)) AS ed5,
							 COUNT(IF((DATEDIFF(date_out, date_in) = 6 AND dispo = 'Admitted to Wards'), 1, NULL)) AS ed6,
							 COUNT(IF((DATEDIFF(date_out, date_in) = 7 AND dispo = 'Admitted to Wards'), 1, NULL)) AS ed7,
							 COUNT(IF((DATEDIFF(date_out, date_in) >= 8 AND DATEDIFF(date_out, date_in) < 14 AND dispo = 'Admitted to Wards'), 1, NULL)) AS ed8,
							 COUNT(IF((DATEDIFF(date_out, date_in) >= 14 AND dispo = 'Admitted to Wards'), 1, NULL)) AS ed9";
				$age_count = "COUNT(IF((ROUND(DATEDIFF(date_in, patients.p_bday)/365) < 18 ), 1, NULL)) AS f1,
							  ...
							  ROUND(SUM(DATEDIFF(date_in, patients.p_bday)/365)/COUNT(er_id)) as averageage,
				...
				$minmax_count = "MAX(ROUND(DATEDIFF(date_in, patients.p_bday)/365)) as maxage,
								 ...
								 ";	
				$sql = "SELECT COUNT(er_id) as total, ".$dispo_count.", ".$hd_count.", ".$m_count.", ".$ed_count.", ".$age_count.", ".$minmax_count." FROM er_census, patients WHERE er_census.p_id = patients.p_id AND date_in >= ? AND date_in <= ?";		
    ...
    $this->db->update('er_census', $data);
	  ...
	  function count_all_pcpdx($date1, $date2, $dx, $ref){
			 $sql = "SELECT COUNT(*) AS numrows, COUNT(IF(dispo = 'Mortality', 1, NULL)) AS mortality, COUNT(IF(refs LIKE ?, 1, NULL)) AS referred, SUM(DATEDIFF(date_out, date_in)) as totaldays FROM er_census WHERE (date_in >= ? AND date_in <= ?) AND pcpdx LIKE ?"; 
