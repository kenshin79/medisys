...
function get_allclean(){
    //$this->db->select('a_id, date_in, date_out, sic, plist, meds, refs, erefs, notes, pcpdx');
    $this->db->select('a_id, abstract, dsummary, sagip, home, c_notes');
...
function clean_adm($aid, $abstract, $dsummary, $sagip, $home, $cnotes ){
    $data = array(
    	...
    	'home'=>clean_form_input($home),
    	'c_notes'=>clean_form_input($cnotes),
...
function get_forms_label($aid, $forms){
      ...
      $this->db->select('a_id, date_in, date_out, location, bed, r_id, sr_id, p_id, service, plist, meds');
      switch ($forms){
            case 0:
...
function count_dispo($serv, $disp, $date1, $date2){
	if (strcmp($serv, "All"))
...
function count_refs($serv, $refs, $date1, $date2){
	if (strcmp($serv, "All"))
		$this->db->where('service', $serv);
        $this->db->where('date_in >=', $date1); 
	$this->db->where('date_in <=', $date2);
...
function count_erefs($serv, $erefs, $date1, $date2){
	if (strcmp($serv, "All"))
		$this->db->where('service', $serv);
        $this->db->where('date_in >=', $date1); 
	$this->db->where('date_in <=', $date2);
...
function get_gm_census($service, $disp){
		 ...
		 if (!strcmp($disp, "Admitted")){
	                if (!strcmp($service, "preop"))
				$this->db->order_by('date_in desc, location asc, bed asc');
			else
...
function edit_one_admission($id){
	...
	$query = $this->db->update('admissions',$data);
	if (!strcmp($data['dispo'], "Admitted") || !strcmp($data['dispo'], "Admitted to MICU") || !strcmp($data['dispo'], "Admitted to Wards"))
	      $adm_status = "Admitted";
	else
	...
	$this->Patient_model->update_one_plist($data['p_id'], $data['plist']); 
        ...
        $handle = fopen($file, 'a+');
        date_default_timezone_set('Asia/Hong_Kong');
        $data = date('m/d/Y h:i:s a', time()).", IP Add: ".$this->session->userdata('ip_address').", user:".$_SERVER['PHP_AUTH_USER'].", task: edit ward admission id#".$id.", GM:".$data['service']." location:".$data['location']." Bed:".$data['bed']." patient#:".$data['p_id'].", dispo: ".$data['dispo']."\r\n"; 
        fwrite($handle, $data);  						  
}
...
*/	  
	 ...
	 //delete an admission
	  ...
	  function insert_one_admission(){  
		    ...
		    $query = $this->db->insert('admissions', $data);
            $id = $this->db->insert_id();
            //log insert task in text file
            $file = "ward_admission_log.txt"; 
            $handle = fopen($file, 'a+');
            date_default_timezone_set('Asia/Hong_Kong');
            $data = date('m/d/Y h:i:s a', time()).", IP Add: ".$this->session->userdata('ip_address').", user:".$_SERVER['PHP_AUTH_USER'].", task: insert ward admission id#".$id." GM:".$data['service']." ".$data['location']." Bed:".$data['bed']." patient id#".$data['p_id']."\r\n"; 
            fwrite($handle, $data);  	
		    return $id;
	  ...
	  function insert_one_admission_bu($aid){
          $ward_data = $this->Admission_model->get_admissiondata_by_id($aid);
		  ...
		  $query = $this->db->insert('admissions_bu', $warddata);	
                  //log backup delete task in text file
                     $file = "ward_admission_log.txt"; 
                     $handle = fopen($file, 'a+');
                     date_default_timezone_set('Asia/Hong_Kong');
                     $data = date('m/d/Y h:i:s a', time()).", IP Add: ".$this->session->userdata('ip_address').", user:".$_SERVER['PHP_AUTH_USER'].", task: backup ward admission id#".$warddata['ab_id']."\r\n"; 
                     fwrite($handle, $data);  
		  return $this->db->insert_id();					  
	  }  
//get ward admission data by id
function get_admissiondata_by_id($aid){
	$this->db->from('admissions');
	$this->db->where('a_id', $aid);
	$query = $this->db->get();
	return $query->result(); 
	...
	function edit_others($aid)
	 ...
	 $others = array(
	  ...
	  $cs_others = implode(",", $others);
        ...
        function get_occupyw1(){
			 $this->db->from('admissions');
			 $this->db->join('patients', 'patients.p_id = admissions.p_id', 'inner');
		         $this->db->join('residents', 'residents.r_id = admissions.r_id', 'inner');
			 $this->db->where('location', 'Ward 1');
			 $this->db->where('type', 'Primary');
	...
	function get_occupyw3(){
			 $this->db->from('admissions');
			 $this->db->join('patients', 'patients.p_id = admissions.p_id', 'inner');
		         $this->db->join('residents', 'residents.r_id = admissions.r_id', 'inner');
			 $this->db->where('location', 'Ward 3');
			 $this->db->where('type', 'Primary');
			 $this->db->where('dispo', 'Admitted');  
			 $this->db->order_by('bed asc');
			 $query = $this->db->get();
